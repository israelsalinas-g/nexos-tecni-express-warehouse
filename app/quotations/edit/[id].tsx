...
export default function EditQuotationScreen() {
  ...
  useEffect(() => {
    async function loadAll() {
      try {
        const [quotation, customersData] = await Promise.all([
          QuotationService.getById(id),
          ProfileService.getAll(),
        ])
        if (!quotation) { Alert.alert('Error', 'Cotización no encontrada'); router.back(); return }
        if (quotation.status !== 'draft') { Alert.alert('Error', 'Solo se pueden editar borradores.'); router.back(); return }

        setCustomers(customersData)
        const customer = customersData.find(c => c.id === quotation.customer_id) ?? null
        setSelectedCustomer(customer)
        setNotes(quotation.notes ?? '')
        setDiscount(quotation.discount ? String(quotation.discount) : '')
        if (quotation.valid_until) setValidUntil(new Date(quotation.valid_until))
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert('Guardado', 'Cotización actualizada.', [
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          {items.length === 0 ? (
            <View style={styles.emptyItems}>
              <Text style={styles.emptyItemsText}>Sin artículos.</Text>
            </View>
          ) : (
            items.map(item => (
...
const styles = StyleSheet.create({
  ...
  textField: { backgroundColor: tokens.colors.bgLight, borderWidth: 1, borderColor: tokens.colors.gray200, borderRadius: tokens.radius.lg, padding: tokens.spacing.md, fontSize: tokens.typography.size.base, color: tokens.colors.gray900 },
  ...
  searchInput: { margin: tokens.spacing.md, backgroundColor: tokens.colors.bgLight, borderWidth: 1, borderColor: tokens.colors.gray200, borderRadius: tokens.radius.lg, paddingHorizontal: tokens.spacing.md, paddingVertical: 10, fontSize: tokens.typography.size.base, color: tokens.colors.gray900 },
